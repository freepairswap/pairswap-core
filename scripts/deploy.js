...
async function adjustPerifpherySourceCode(address) {
  ...
  const PATH_GET = "../pair-swap-periphery/contracts/libraries/PancakeLibraryTemplate.sol"
  const PATH_PUT = "../pair-swap-periphery/contracts/libraries/PancakeLibrary.sol"

  const contract = await ethers.getContractAt("PairFactory", address)
...
async function main() {
  ...
  PairFactory = await deploy("PairFactory", [admins._devaddr])

  await adjustPerifpherySourceCode(PairFactory)
